...
Django 5 task management API expanding with WebSocket notifications and a React/TypeScript Kanban frontend.
...
|-----------|-------|--------------------|
| build, fix, refactor, unit tests | @developer | work item + relevant src + specs |
| design, API contracts, component specs | @designer | work item + constraints + existing models |
| integration tests, e2e tests, test plans | @qa | work item + test strategy + src |
...
1. Run full verification: `cd backend && ruff check . && python -m pytest && python manage.py check`
2. `git tag v{X.Y}`
3. Clear "Done" section in PROJECT.md
4. Bump version in State section
5. Seed next work items

Versions are cheap. Ship often.

## Conventions

- Python: Django 5, DRF, ruff for linting, pytest for tests
- Auth: simplejwt — all new endpoints require JWT unless explicitly public
- DB: PostgreSQL, Django ORM migrations
- Async: Django Channels + Redis for WebSocket layer
- Frontend: React + TypeScript (conventions TBD until scaffolded)
- Models use `created_at`/`updated_at` timestamps via `TimeStampedModel` base
- Serializers live alongside their viewsets in the same app
- URL routing: DRF routers in each app's `urls.py`, included from project `urls.py`
- Tests: pytest-django, factories with factory_boy
- Commits: one logical change each, verified before push
